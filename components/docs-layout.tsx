...
interface DocsLayoutProps {
  children: React.ReactNode | ((projectId: string, activeSection: string) => React.ReactNode)
}
...
export function DocsLayout({ children }: DocsLayoutProps) {
  ...
  const [selectedProject, setSelectedProject] = useState(() => {
    const defaultProject = projects.mainnet[0]
    console.log("[v0] DocsLayout initializing with default project:", defaultProject)
    return defaultProject
  })
  const [projectSelectorOpen, setProjectSelectorOpen] = useState(false)
  const [activeSection, setActiveSection] = useState("installation") // Default to installation section
  const [mounted, setMounted] = useState(false)

  useEffect(() => {
    setMounted(true)
    console.log("[v0] DocsLayout mounted, selectedProject:", selectedProject)
  }, [selectedProject])

  const handleProjectChange = (project: (typeof projects.mainnet)[0]) => {
    console.log("[v0] DocsLayout changing project to:", project)
  ...
  console.log("[v0] DocsLayout rendering with selectedProject.id:", selectedProject?.id)
      ...
      <div className="container flex-1 items-start md:grid md:grid-cols-[260px_minmax(0,1fr)] md:gap-8 lg:grid-cols-[280px_minmax(0,1fr)] lg:gap-12">
        ...
        <main className="relative py-8 lg:py-10">
          <div className="mx-auto w-full min-w-0 animate-fade-in">
            {typeof children === "function" ? children(selectedProject.id, activeSection) : children}
          </div>
        </main>
      </div>
